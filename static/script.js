// To display predictions, this app has:
// 1. A video that shows a feed from the user's webcam
// 2. A canvas that appears over the video and shows predictions
// When the page loads, a user is asked to give webcam permission.
// After this happens, the model initializes and starts to make predictions
// On the first prediction, an initialiation step happens in detectFrame()
// to prepare the canvas on which predictions are displayed.

// БЛОК ГЛОБАЛЬНЫХ ПЕРЕМЕННЫХ И НАСТРОЕК

// Хранилище цветов для bounding box по классам объектов
var bounding_box_colors = {};
// Порог уверенности модели для отображения предсказаний (60% по умолчанию)
var user_confidence = 0.6;

// Палитра доступных цветов для bounding box'ов
// Update the colors in this list to set the bounding box colors
var color_choices = [
  "#C7FC00",
  "#FF00FF",
  "#8622FF",
  "#FE0056",
  "#00FFCE",
  "#FF8000",
  "#00B7EB",
  "#FFFF00",
  "#0E7AFE",
  "#FFABAB",
  "#0000FF",
  "#CCCCCC",
];
// Флаг, указывающий, был ли canvas уже проинициализирован
var canvas_painted = false;

// Получение элемента canvas и его 2D контекста для рисования
var canvas = document.getElementById("video_canvas");
var ctx = canvas.getContext("2d");

// Экземпляр движка инференса Roboflow
const inferEngine = new inferencejs.InferenceEngine();
// Идентификатор воркера модели (будет получен после загрузки модели)
var modelWorkerId = null;

// БЛОК ОСНОВНОГО ЦИКЛА ОБНАРУЖЕНИЯ (DETECT FRAME)
function detectFrame() {
  // При первом запуске инициализируется canvas
  // При каждом запуске выполняется инференс на кадре из видео
  // Для каждого кадра отрисовываются bounding box'ы

  // Если модель еще не загружена, просто запрашиваем следующий кадр
  if (!modelWorkerId) return requestAnimationFrame(detectFrame);
  // КЛЮЧЕВАЯ ФУНКЦИЯ: выполнение инференса на текущем кадре видео
  inferEngine.infer(modelWorkerId, new inferencejs.CVImage(video)).then(function(predictions) {
     // Инициализация canvas при первом предсказании
    if (!canvas_painted) {
      var video_start = document.getElementById("video1");
      // Позиционирование canvas поверх видео
      canvas.top = video_start.top;
      canvas.left = video_start.left;
      canvas.style.top = video_start.top + "px";
      canvas.style.left = video_start.left + "px";
      canvas.style.position = "absolute";
      video_start.style.display = "block";
      canvas.style.display = "absolute";
      canvas_painted = true;
      // Скрытие индикатора загрузки
      var loading = document.getElementById("loading");
      loading.style.display = "none";
    }
    // Рекурсивный вызов для обработки следующего кадра
    requestAnimationFrame(detectFrame);
    // Очистка canvas перед новой отрисовкой
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    // Отрисовка bounding box'ов, если видео существует
    if (video) {
      drawBoundingBoxes(predictions, ctx)
    }
  });
}
// БЛОК ОТРИСОВКИ BOUNDING BOX
function drawBoundingBoxes(predictions, ctx) {
  // For each prediction, choose or assign a bounding box color choice,
  // then apply the requisite scaling so bounding boxes appear exactly
  // around a prediction.

  // If you want to do anything with predictions, start from this function.
  // For example, you could display them on the web page, check off items on a list,
  // or store predictions somewhere.
  // Для каждого предсказания выбирается или назначается цвет bounding box'а
  // Затем применяется масштабирование для правильного позиционирования

  // Перебор всех предсказаний модели
  for (var i = 0; i < predictions.length; i++) {
    var confidence = predictions[i].confidence;

    console.log(user_confidence)
     // ФИЛЬТРАЦИЯ: пропуск предсказаний с низкой уверенностью
    if (confidence < user_confidence) {
      continue
    }
     // ФИЛЬТРАЦИЯ: пропуск предсказаний с низкой уверенностью
    if (predictions[i].class in bounding_box_colors) {
      // Использование существующего цвета для этого класса
      ctx.strokeStyle = bounding_box_colors[predictions[i].class];
    } else {
      // Выбор случайного цвета для нового класса
      var color =
        color_choices[Math.floor(Math.random() * color_choices.length)];
      ctx.strokeStyle = color;
      // Удаление использованного цвета из палитры
      // remove color from choices
      color_choices.splice(color_choices.indexOf(color), 1);
      // Сохранение цвета для будущих предсказаний этого класса
      bounding_box_colors[predictions[i].class] = color;
    }

    var prediction = predictions[i];
    // КЛЮЧЕВЫЕ РАСЧЕТЫ: преобразование координат bounding box
    // Roboflow возвращает центр (x,y), ширину и высоту
    var x = prediction.bbox.x - prediction.bbox.width / 2;
    var y = prediction.bbox.y - prediction.bbox.height / 2;
    var width = prediction.bbox.width;
    var height = prediction.bbox.height;
    // Отрисовка прямоугольника
    ctx.rect(x, y, width, height);

    ctx.fillStyle = "rgba(0, 0, 0, 0)";
    ctx.fill();
    // Отрисовка контура
    ctx.fillStyle = ctx.strokeStyle;
    ctx.lineWidth = "4";
    ctx.strokeRect(x, y, width, height);
    // КЛЮЧЕВАЯ ФУНКЦИЯ: отображение текста с классом объекта и процентом уверенности
    ctx.font = "25px Arial";
    ctx.fillText(prediction.class + " " + Math.round(confidence * 100) + "%", x, y - 10);
  }
}

// БЛОК ИНИЦИАЛИЗАЦИИ ВЕБ-КАМЕРЫ И МОДЕЛИ
function webcamInference() {
  // Запрос доступа к веб-камере и запуск основного приложения

  // Показ индикатора загрузки
  var loading = document.getElementById("loading");
  loading.style.display = "block";
  // КЛЮЧЕВАЯ ФУНКЦИЯ: запрос доступа к веб-камере
  navigator.mediaDevices
    .getUserMedia({ video: { facingMode: "environment" } }) // "environment" = задняя камера
    .then(function(stream) {
      // Создание элемента video для потока с камеры
      video = document.createElement("video");
      video.srcObject = stream;
      video.id = "video1";

      // hide video until the web stream is ready
      video.style.display = "none"; // Скрыто до готовности
      video.setAttribute("playsinline", "");
      // Вставка видео после canvas
      document.getElementById("video_canvas").after(video);
      // Запуск воспроизведения после загрузки метаданных
      video.onloadedmetadata = function() {
        video.play();
      }

      // Настройка размеров при начале воспроизведения
      video.onplay = function() {
        height = video.videoHeight;
        width = video.videoWidth;

        // scale down video by 0.75
        // Установка фиксированных размеров для отображения
        video.width = width;
        video.height = height;
        video.style.width = 640 + "px";
        video.style.height = 480 + "px";

        canvas.style.width = 640 + "px";
        canvas.style.height = 480 + "px";
        canvas.width = width;
        canvas.height = height;

        document.getElementById("video_canvas").style.display = "block";
      };

      ctx.scale(1, 1);

      // Load the Roboflow model using the publishable_key set in index.html
      // and the model name and version set at the top of this file
       // КЛЮЧЕВАЯ ФУНКЦИЯ: загрузка модели Roboflow
      inferEngine.startWorker(MODEL_NAME, MODEL_VERSION, publishable_key, [{ scoreThreshold: CONFIDENCE_THRESHOLD }])
        .then((id) => {
          modelWorkerId = id;
          // Start inference
          detectFrame();
        });
    })
    .catch(function(err) {
      console.log(err);
    });
}
// БЛОК ОБРАБОТЧИКОВ ПОЛЬЗОВАТЕЛЬСКОГО ВВОДА
// Функция обновления порога уверенности на основе значения слайдера
function changeConfidence () {
  user_confidence = document.getElementById("confidence").value / 100;
}
// Регистрация обработчика события для слайдера
document.getElementById("confidence").addEventListener("input", changeConfidence);
// БЛОК ЗАПУСКА ПРИЛОЖЕНИЯ
// Запуск основного процесса инференса
webcamInference();